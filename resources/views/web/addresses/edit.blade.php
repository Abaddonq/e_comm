...
<div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
    <div class="max-w-2xl mx-auto">
        <h1 class="text-2xl font-bold text-gray-900 mb-6">Edit Address</h1>

        <form action="{{ route('addresses.update', $address->id) }}" method="POST" class="bg-white rounded-lg shadow p-6">
            @csrf
            @method('PUT')
            ...
            <div class="space-y-4">
                <div>
                    ...
                    <input type="text" name="full_name" id="full_name" value="{{ old('full_name', $address->full_name) }}" required
                ...
                <div class="grid grid-cols-2 gap-4">
                    ...
                    <div>
                        ...
                        <select name="country" id="country" required
                            class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500">
                            <option value="TR" {{ old('country', $address->country) == 'TR' ? 'selected' : '' }}>Turkey</option>
                            ...
                            <option value="ES" {{ old('country', $address->country) == 'ES' ? 'selected' : '' }}>Spain</option>
                            <option value="NL" {{ old('country', $address->country) == 'NL' ? 'selected' : '' }}>Netherlands</option>
                        </select>
                        @error('country')
                            <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                        @enderror
